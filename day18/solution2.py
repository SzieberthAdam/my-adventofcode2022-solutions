...
while reduced:
    print("w")
    reduced = False
    for cube in list(centers2):
        S2 = connected2.get(cube, set())
        S3 = connected3.get(cube, set())
        if len(S2) + len(S3) != 6:
            print(cube)
            for cube2 in S2:
                if cube2 in connected2: del connected2[cube2]
                if cube2 in connected3: del connected3[cube2]
                centers2.remove(cube2)
            if cube in connected2: del connected2[cube]
            if cube in connected3: del connected3[cube]
